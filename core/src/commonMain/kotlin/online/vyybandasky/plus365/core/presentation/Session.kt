...
data class Session(
    ...
    /** Lend from the pool at the house rate. Two entries, both pending. */
    fun lend(borrower: MemberId, principalCents: Long): Session {
        ...
        return when (val r = book.disburseLoan(
            ...
            is Decision.Allowed -> copy(
                ...
                idCounter = idCounter + 1,
                notice = Notice.Info(
                    "Loan recorded with ${r.value.entries.size} entries, both awaiting confirmation.",
                ),
            )
            is Decision.Refused -> copy(notice = Notice.Refused(r.refusal.message))
        }
